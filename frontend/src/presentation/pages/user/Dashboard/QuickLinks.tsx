...
export default function QuickLinks() {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl sm:rounded-3xl shadow-xl ${styles.backgroundSecondary} group hover:shadow-2xl transition-all duration-500`}>
      ...
      <div className={`absolute -top-4 sm:-top-8 -left-4 sm:-left-8 w-16 h-16 sm:w-32 sm:h-32 rounded-full bg-gradient-to-br ${styles.orb.secondary} blur-xl sm:blur-2xl animate-pulse delay-1000`}></div>
      <div className={`absolute top-2 sm:top-4 right-2 sm:right-4 w-10 h-10 sm:w-20 sm:h-20 ${styles.pattern.primary} rounded-xl sm:rounded-2xl rotate-12 group-hover:rotate-45 transition-transform duration-700`}></div>
      <div className={`absolute bottom-3 sm:bottom-6 left-3 sm:left-6 w-4 h-4 sm:w-8 sm:h-8 bg-gradient-to-br ${styles.pattern.secondary} rounded-full group-hover:scale-150 transition-transform duration-500`}></div>
        ...
        <div className="flex items-center space-x-3 sm:space-x-4 mb-4 sm:mb-6">
          <div className="relative">
            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-br ${styles.accent} flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:scale-110`}>
              ...
              <FaBookmark size={16} className="sm:w-5 sm:h-5 text-white relative z-10" />
            </div>
            ...
            >
              ...
              <div className="relative z-10 flex items-center justify-between">
                <div className="flex items-center space-x-3 sm:space-x-4">
                  <div className="relative">
                    <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-gradient-to-br ${link.color} flex items-center justify-center shadow-md group-hover/link:shadow-lg transition-all duration-300 group-hover/link:scale-110`}>
                    ...
                    <div className={`absolute -inset-1 bg-gradient-to-br ${link.color} opacity-30 rounded-lg sm:rounded-xl blur group-hover/link:opacity-50 transition-opacity duration-300`}></div>
                  </div>
                  <div className="flex items-center space-x-2 sm:space-x-3">
                    <span className={`${styles.icon.secondary} text-xl sm:text-2xl`}>{link.emoji}</span>
                    <span className={`text-sm sm:text-base font-semibold ${styles.textPrimary} group-hover/link:text-gray-900 transition-colors duration-200`}>
                      {link.text}
                    </span>
                  </div>
                </div>
              ...
              <div className={`absolute bottom-0 left-0 h-0.5 sm:h-1 w-0 bg-gradient-to-r ${styles.accent} group-hover/link:w-full transition-all duration-300 rounded-full`}></div>
            </button>
          ))}
        </div>
      </div>
