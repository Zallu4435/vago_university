...
class AdminDashboardService {
  ...
  async refreshDashboard(): Promise<DashboardData> {
    try {
      ...
      throw new Error(error.response?.data?.error || 'Failed to refresh dashboard');
    }
  }

  async dismissAlert(alertId: string): Promise<void> {
    try {
      await httpClient.delete(`/admin/dashboard/alerts/${alertId}`);
    } catch (error: any) {
      throw new Error(error.response?.data?.error || 'Failed to dismiss alert');
    }
  }

  async markActivityAsRead(activityId: string): Promise<void> {
    try {
      await httpClient.patch(`/admin/dashboard/activities/${activityId}/read`);
    } catch (error: any) {
      throw new Error(error.response?.data?.error || 'Failed to mark activity as read');
    }
  }
}
