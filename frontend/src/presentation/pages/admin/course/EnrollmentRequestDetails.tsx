...
// Reuse StatusBadge and InfoCard from TeamRequestDetailsModal
...
const InfoCard: React.FC<InfoCardProps> = ({ icon: Icon, label, value, accent = 'purple', highlight = false }) => {
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl border backdrop-blur-sm p-6 transition-all duration-500 hover:scale-110 hover:shadow-2xl transform-gpu ${colors.card} ${colors.hover} ${highlight ? 'ring-2 ring-white/20 shadow-xl' : ''}`}
    >
        ...
        <div className="relative bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900 border border-purple-500/40 rounded-3xl shadow-2xl">
          ...
          <div className="relative z-10 p-6 max-h-[calc(100vh-200px)] overflow-y-auto">
            ...
            <div className="flex flex-wrap items-center justify-between mb-10 gap-6">
              ...
              <div className="flex items-center space-x-8 text-sm font-medium">
                ...
                <div className="flex items-center space-x-3 px-4 py-2 bg-blue-500/10 rounded-full border border-blue-400/20">
                  ...
                  <span className="text-blue-200">{course?.credits} Credits</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-10">
              ...
              <InfoCard icon={Info} label="Specialization" value={course?.specialization} accent="green" />
              <InfoCard icon={User} label="Faculty" value={course?.faculty} accent="pink" />
              <InfoCard icon={Calendar} label="Term" value={course?.term} accent="cyan" />
              <InfoCard
                icon={Calendar}
                label="Last Updated"
                ...
                accent="gold"
              />
              <InfoCard icon={Book} label="Credits" value={course?.credits} accent="purple" />
              <InfoCard icon={Info} label="Course ID" value={course?.id} accent="blue" />
            </div>
            ...
            <div className="mb-10">
              <div className="bg-gradient-to-r from-emerald-900/30 to-teal-900/30 rounded-2xl p-8 border border-emerald-500/30 backdrop-blur-sm shadow-xl">
                <div className="flex items-center space-x-3 mb-6">
                  ...
                  <h3 className="text-xl font-bold text-white">Reason for Enrollment</h3>
                </div>
                <div className="bg-gray-800/40 rounded-xl p-6 border border-gray-600/20">
                  <p className="text-gray-200 leading-relaxed text-lg">{request?.reason}</p>
                </div>
              </div>
            </div>
