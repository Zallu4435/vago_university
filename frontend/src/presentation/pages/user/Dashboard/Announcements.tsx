...
export default function Announcements({ announcements }) {
  ...
  useEffect(() => {
    console.log('Announcements theme styles:', styles);
  }, [styles]);
      ...
      <div className="relative z-10 p-7">
        ...
        <div className="space-y-4">
          {announcements.map((item, index) => (
            ...
            >
              <div className={`absolute -inset-0.5 bg-gradient-to-r ${styles.orb.secondary} rounded-2xl blur transition-all duration-300 group-hover/item:opacity-20`}></div>
              <div className="relative z-10 flex items-start justify-between">
                ...
                <FaArrowRight className={`${styles.icon.primary} opacity-0 group-hover/item:opacity-100 group-hover/item:translate-x-1 transition-all duration-300`} size={14} />
              </div>
            </div>
          ))}
