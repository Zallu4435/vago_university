                ...
                <li key={item.id}>
                  ...
                  >
                    <Icon className={`w-4 h-4 mr-3 transition-colors duration-200 ${
                      isActive ? 'text-purple-600' : 'text-slate-500 group-hover:text-slate-600'
                    }`} />
                    <span className="font-medium text-sm">{item.label}</span>
                    {isActive && (
                      <div className="absolute right-3 w-2 h-2 bg-purple-500 rounded-full"></div>
                    )}
                  </button>
                </li>
              );
          ...
          <div className="flex items-center mb-4 p-3 bg-slate-50 rounded-lg">
            <div className="w-10 h-10 rounded-lg bg-purple-100 border border-purple-200 flex items-center justify-center mr-3 shadow-sm">
              {user?.profilePicture ? (
                ...
                <FaUserAlt className="w-5 h-5 text-purple-600" />
              )}
            </div>
