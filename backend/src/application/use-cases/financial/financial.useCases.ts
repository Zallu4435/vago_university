...
export class GetStudentFinancialInfo {
  async execute(studentId: string): Promise<any> {
    try {
      ...
      const info = await StudentFinancialInfoModel.findOne({
        studentId,
      }).lean();
      ...
      if (!info) {
        return {
          success: false,
          message: "No financial information found for this student",
        };
      }
      ...
      return {
        success: false,
        message: `Failed to fetch financial info: ${err.message}`,
      };
    }
  }
}
...
export class GetCurrentCharges {
  async execute(studentId: string, term?: string): Promise<any[]> {
    try {
      ...
      if (!charges.length) return []; // Return empty array for no charges
...
export class GetPaymentHistory {
  async execute(
    ...
    try {
      ...
      if (!payments.length) return []; // Return empty array for no payments
  ...
  async execute(studentId: string, input: PaymentInput): Promise<any> {
    try {
      ...
      const payment = new PaymentModel({
        ...
        method: input.method,
        amount: input.amount,
        status: "Completed", // Simplified; integrate with payment gateway in production
      });
      await payment.save();
      return {
        id: payment._id.toString(),
...
export class GetFinancialAidApplications {
  async execute(studentId: string, status?: string): Promise<any[]> {
    try {
      ...
      const applications = await FinancialAidApplicationModel.find(
        query
      ).lean();
      if (!applications.length) return []; // Return empty array for no applications
      ...
      throw new Error(
        `Failed to fetch financial aid applications: ${err.message}`
      );
    }
  }
}
...
export class GetAvailableScholarships {
  async execute(filters: { status?: string; term?: string }): Promise<any[]> {
    try {
      ...
      if (!scholarships.length) return []; // Return empty array for no scholarships
...
export class GetScholarshipApplications {
  async execute(studentId: string, status?: string): Promise<any[]> {
    try {
      ...
      throw new Error(
        `Failed to fetch scholarship applications: ${err.message}`
      );
    }
  }
}
...
export class UpdateFinancialAidApplication {
  async execute(
    ...
    try {
      ...
      if (!application)
        throw new Error(
          "No financial aid application found with the specified ID"
        );
      return {
        id: application._id.toString(),
        studentId: application.studentId.toString(),
      ...
      throw new Error(
        `Failed to update financial aid application: ${err.message}`
      );
    }
  }
}
...
export class UpdateScholarshipApplication {
  async execute(
    ...
    try {
      ...
      if (!application)
        throw new Error(
          "No scholarship application found with the specified ID"
        );
      return {
        id: application._id.toString(),
        scholarshipId: application.scholarshipId.toString(),
...
export const updateFinancialAidApplication =
  new UpdateFinancialAidApplication();
