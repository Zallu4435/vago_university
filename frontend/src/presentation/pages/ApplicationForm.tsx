...
interface FormData {
  ...
  declaration?: DeclarationSection; // ✅ add this
...
export const ApplicationForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        setFormData({ personalInfo: parsedData.personalInfo });
      } catch (error) {
  ...
  const handleUpdateEducation = (data: EducationData) => {
  ...
  const handleSaveCurrentTab = () => {
    if (activeTab === 'personalDetails') {
      if (formData.personalInfo) {
        console.log("personalInfo data", formData.personalInfo)
      ...
      if (formData.choiceOfStudy && formData.choiceOfStudy.length > 0) {
        console.log("choice of study data", formData.choiceOfStudy)
        localStorage.setItem('applicationFormData', JSON.stringify(formData));
        handleNextTab();
      } else {
      ...
      if (formData.achievements) {
        console.log("achievement data", formData.achievements)
        localStorage.setItem('applicationFormData', JSON.stringify(formData));
        handleNextTab();
      } else {
      ...
      if (formData.otherInformation) {
        console.log('other_info data', formData.otherInformation)
        localStorage.setItem('applicationFormData', JSON.stringify(formData));
        handleNextTab();
      } else {
      ...
      if (formData.documents) {
        console.log("document data", formData.documents)
        localStorage.setItem('applicationFormData', JSON.stringify(formData));
        handleNextTab();
      } else {
      ...
      <div className="bg-white shadow-sm p-6 rounded-xl border border-cyan-100 relative overflow-hidden">
        ...
        {activeTab === 'declaration' && <Declaration
          ...
          onChange={(data) => {
            ...
            localStorage.setItem('applicationFormData', JSON.stringify({ ...formData, declaration: data }));
          }}
        />}
        <div className="flex justify-between mt-8 border-t border-cyan-50 pt-6">
          ...
          {activeTab !== 'declaration' ? (
            ...
            <button
              className={`px-8 py-3 bg-gradient-to-r from-cyan-400 to-cyan-600 text-white rounded-lg font-bold relative overflow-hidden group transition-all duration-300 shadow-sm ${!declaration.privacyPolicy ? 'opacity-50 cursor-not-allowed' : 'hover:from-cyan-500 hover:to-cyan-700'
                }`}
              disabled={!declaration.privacyPolicy}
              onClick={() => {
