...
class EventsController {
  async getEvents(req: Request, res: Response, next: NextFunction) {
    try {
      const { page = '1', limit = '10', type = 'all', status = 'all', organizer = 'all' } = req.query;
  ...
  async getEventRequests(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      console.log(`Received GET /api/admin/events/requests with filters:`, {
        page,
        limit,
        type,
        status,
  ...
  async rejectEventRequest(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      console.log(`Received POST /api/admin/events/requests/${id}/reject with reason:`, reason);
      ...
      await rejectEventRequest.execute({ id, reason });
  ...
  async rejectParticipant(req: Request, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const { reason } = req.body;
      console.log(`Received POST /api/admin/events/participants/${id}/reject with reason:`, reason);
